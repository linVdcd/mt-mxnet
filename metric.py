    ...
    def update(self, labels, preds):
        ...
        for i in range(len(labels)):
            ...
            if self.num is None:
                ...
                self.num_inst[i] +=len(label)-np.sum(label==1000)
    ...
    def get(self):
        if self.num is None:
            ...
            for i in range(self.num):
                name.append('%s-task%d' % (self.name, i))
                value.append(0.0 if self.num_inst[i] == 0
            else float(self.sum_metric[i]) / float(self.num_inst[i]))
            return name,value
